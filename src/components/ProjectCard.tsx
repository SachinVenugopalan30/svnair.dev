...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, index }): ReactElement => {
        ...
        <div className="flex gap-3 pt-2">
          {githubUrl && (
            ...
            >
              ...
              <span className="relative z-10 tracking-wide font-medium">Code</span>
            </motion.a>
          )}
          ...
          {liveUrl && (
            <motion.a 
              ...
              whileHover={{ scale: 1.05, y: -2 }}
              whileTap={{ scale: 0.95 }}
              className="px-4 py-2 rounded-full text-xs font-semibold transition-all duration-300 relative overflow-hidden group/btn border backdrop-blur-md no-underline text-white bg-white/10 border-white/20 hover:bg-gradient-to-r hover:from-scarlet/90 hover:to-scarlet hover:border-scarlet/60 hover:shadow-lg hover:shadow-scarlet/20 inline-flex items-center gap-2 flex-1 justify-center"
              style={{ textDecoration: 'none' }}
            >
              {/* Animated background gradient - only on hover */}
              ...
              <span className="relative z-10 tracking-wide font-medium">Live Demo</span>
            </motion.a>
          )}
        </div>
      </div>
